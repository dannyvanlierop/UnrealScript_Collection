...
*******************************************************************************
* Rx_Mutator_GiveSkin_Controller                                          *
******************************************************************************/
 
class Rx_Mutator_GiveSkin_Controller extends Rx_Controller;
 
var string team;
var string ReferenceArray[33];
//var array<string> ReferenceArray;
var SkeletalMesh MeshArray[33];
var int GiveSkinLoopTier;

reliable server function GeneralServer(int MeshIndex)
{
    local Rx_Mutator_GiveSkin_Controller PC;

	Pawn.Mesh.SetSkeletalMesh(MeshArray[MeshIndex]);
    foreach WorldInfo.AllControllers(class'Rx_Mutator_GiveSkin_Controller', PC)
...
exec function GiveSkin(string SoldierName)
{
	local int x;
...
local int x;
	...
	GiveSkinLoopTier = GiveSkinLoopTier + 1;
	if(GiveSkinLoopTier <= 34)
		settimer(5,false,'GiveSkinLoop');
	else
		GiveSkinLoopTier = 0;
}
